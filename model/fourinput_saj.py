...
#=====未对ecg做rfft,所以特征维度不会变为1/2
        ...
        self.conv_layer = nn.Conv1d(in_channels=128, out_channels=128, kernel_size=5, stride=2, padding=2)  # 示例参数
        self.pool = nn.AdaptiveAvgPool1d(256)
        self.cxrpro=nn.Linear(245, 256)
        self.attention_layer = SelfAttention(in_channels=128)
        self.attention_layer1 = SelfAttention(in_channels=128)
        self.attention_layer2 = SelfAttention(in_channels=128)
        self.fc1 = nn.Linear(384, 256)  # 输入维度为 512（模态 A 和 B 的拼接）
...
#换max为project到256维度
        ...
        print(f'ecg11 {text.shape}')#[bs,128]
        print(f'cxr11 {image.shape}')#[bs,128]
        text1=text[:,None,:]
        image1=image[:,None,:]
        
        feats=torch.cat([text1,image1],dim=1)#[bs,2,128]
        # attention_output1 = self.attention_layer1(feats)  # 形状为 [batch_size, 2, 128]
        # # print(f'after attention1 {attention_output1.shape}')
        # attention_output2 = self.attention_layer2(attention_output1)  # 第二层自注意力
        # attention_output = attention_output2[:, -1, :]  # 形状为 [batch_size, 256]
        # print(f'attention_output {attention_output.shape}')
        # combined_features = torch.cat((text1[:,0, :], image1[:, 0, :], attention_output), dim=1)  # 形状为 [batch_size, 512]
        # # print(f'combined_features {combined_features.shape}')#[bs,384]
        ...
        # text1 = text.permute(0, 2, 1)  # 变为 [bs, 4112, 128]
        # print(f'after conv ecg 0 {text1.shape}')#[bs,4112,128]
        #text[bs,128,4112]
        # text=self.conv_layer(text)
        # print(f'after conv ecg {text.shape}')#[bs,128,2056]
        # text=self.pool(text)
        # image=self.cxrpro(image)
        # t_i=torch.cat([text,image],dim=2)
        # print(f'final cat {t_i.shape}')#[bs,128,512]
        # print(f'after pool ecg {text.shape}')#[bs,128,256]
        # print(f'after max  ecg {text.shape}')#[bsm128]
        # print(f'after max  cxr {image.shape}')#[bsm128]
        f = self.fusion(text, image)  # (batch, d_model)
        # f=self.attention_layer(t_i)
        # f=torch.max(f,dim=2)[0]
        # print(f'f1 {f.shape}')#[bs,128]
        # f=f.permute(0,2,1)
        # print(f'f2 {f.shape}')
        outputs = self.mlp(f)


        return text1, image1, outputs, f
